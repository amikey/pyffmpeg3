from ..util import enum

from ctypes import (c_char_p, c_int, c_void_p, POINTER, c_uint64, Structure,
                    c_uint8, c_uint)

# this is probably broken, but I don't know what else to do.
c_enum = c_int
c_int_p = POINTER(c_int)
c_uint64_p = POINTER(c_uint64)
class AVCodec(Structure):
    _fields_ = [
        ('name', c_char_p),
        ('long_name', c_char_p),
        ('type', c_enum),
        ('id', c_enum),
        ('capabilities', c_int),
        ('supported_framerates', c_void_p),  # AVRational *
        ('pix_fmts', c_int_p), # enum AVPixelFormat *
        ('supported_samplerates', c_int_p),
        ('sample_fmts', c_int_p),  # const enum AVSampleFormat *
        ('channel_layouts', c_uint64_p),  #         ///< array of support channel layouts, or NULL if unknown. array is terminated by 0
        ('max_lowres', c_uint8),  #                 ///< maximum value for lowres supported by the decoder
        ('priv_class', c_void_p),  # const AVClass *priv_class;              ///< AVClass for the private context
        ('profiles', c_void_p),  #const AVProfile *profiles;              ///< array of recognized profiles, or NULL if unknown, array is terminated by {FF_PROFILE_UNKNOWN}
        ('wrapper_name', c_char_p),  # const char *
    ]
AVCodec_p = POINTER(AVCodec)


class AVCodecContext(Structure):
    _fields_ = [
        ('av_class', c_void_p), # const AVClass *av_class;
        ('log_level_offset', c_int),  # int log_level_offset;
        ('codec_type', c_int), # enum AVMediaType /* see AVMEDIA_TYPE_xxx */
        ('codec', AVCodec_p),
        ('codec_id', c_int),  # /* see AV_CODEC_ID_xxx */
        ('codec_tag', c_uint)
        ]
AVCodecContext_p = POINTER(AVCodecContext)


# This was generated by copy pasting from the file `avcodec.h`
# We then stripped the c-style prefix `AV_CODEC_ID`
# and seperated the codecs by blocks because enumerate isn't obvious in python
# I also removed "dummy" codec names that shouldn't really be used
# by the high level python interface
codecs = {}

codecs.update(enum(0x0, [
    'NONE',

    # video codecs
    'MPEG1VIDEO',
    'MPEG2VIDEO', # < preferred ID for MPEG-1/2 video decoding
    'H261',
    'H263',
    'RV10',
    'RV20',
    'MJPEG',
    'MJPEGB',
    'LJPEG',
    'SP5X',
    'JPEGLS',
    'MPEG4',
    'RAWVIDEO',
    'MSMPEG4V1',
    'MSMPEG4V2',
    'MSMPEG4V3',
    'WMV1',
    'WMV2',
    'H263P',
    'H263I',
    'FLV1',
    'SVQ1',
    'SVQ3',
    'DVVIDEO',
    'HUFFYUV',
    'CYUV',
    'H264',
    'INDEO3',
    'VP3',
    'THEORA',
    'ASV1',
    'ASV2',
    'FFV1',
    '4XM',
    'VCR1',
    'CLJR',
    'MDEC',
    'ROQ',
    'INTERPLAY_VIDEO',
    'XAN_WC3',
    'XAN_WC4',
    'RPZA',
    'CINEPAK',
    'WS_VQA',
    'MSRLE',
    'MSVIDEO1',
    'IDCIN',
    '8BPS',
    'SMC',
    'FLIC',
    'TRUEMOTION1',
    'VMDVIDEO',
    'MSZH',
    'ZLIB',
    'QTRLE',
    'TSCC',
    'ULTI',
    'QDRAW',
    'VIXL',
    'QPEG',
    'PNG',
    'PPM',
    'PBM',
    'PGM',
    'PGMYUV',
    'PAM',
    'FFVHUFF',
    'RV30',
    'RV40',
    'VC1',
    'WMV3',
    'LOCO',
    'WNV1',
    'AASC',
    'INDEO2',
    'FRAPS',
    'TRUEMOTION2',
    'BMP',
    'CSCD',
    'MMVIDEO',
    'ZMBV',
    'AVS',
    'SMACKVIDEO',
    'NUV',
    'KMVC',
    'FLASHSV',
    'CAVS',
    'JPEG2000',
    'VMNC',
    'VP5',
    'VP6',
    'VP6F',
    'TARGA',
    'DSICINVIDEO',
    'TIERTEXSEQVIDEO',
    'TIFF',
    'GIF',
    'DXA',
    'DNXHD',
    'THP',
    'SGI',
    'C93',
    'BETHSOFTVID',
    'PTX',
    'TXD',
    'VP6A',
    'AMV',
    'VB',
    'PCX',
    'SUNRAST',
    'INDEO4',
    'INDEO5',
    'MIMIC',
    'RL2',
    'ESCAPE124',
    'DIRAC',
    'BFI',
    'CMV',
    'MOTIONPIXELS',
    'TGV',
    'TGQ',
    'TQI',
    'AURA',
    'AURA2',
    'V210X',
    'TMV',
    'V210',
    'DPX',
    'MAD',
    'FRWU',
    'FLASHSV2',
    'CDGRAPHICS',
    'R210',
    'ANM',
    'BINKVIDEO',
    'IFF_ILBM',
    'KGV1',
    'YOP',
    'VP8',
    'PICTOR',
    'ANSI',
    'A64_MULTI',
    'A64_MULTI5',
    'R10K',
    'MXPEG',
    'LAGARITH',
    'PRORES',
    'JV',
    'DFA',
    'WMV3IMAGE',
    'VC1IMAGE',
    'UTVIDEO',
    'BMV_VIDEO',
    'VBLE',
    'DXTORY',
    'V410',
    'XWD',
    'CDXL',
    'XBM',
    'ZEROCODEC',
    'MSS1',
    'MSA1',
    'TSCC2',
    'MTS2',
    'CLLC',
    'MSS2',
    'VP9',
    'AIC',
    'ESCAPE130',
    'G2M',
    'WEBP',
    'HNM4_VIDEO',
    'HEVC',
    'FIC',
    'ALIAS_PIX',
    'BRENDER_PIX',
    'PAF_VIDEO',
    'EXR',
    'VP7',
    'SANM',
    'SGIRLE',
    'MVC1',
    'MVC2',
    'HQX',
    'TDSC',
    'HQ_HQA',
    'HAP',
    'DDS',
    'DXV',
    'SCREENPRESSO',
    'RSCC',
    'AVS2',
    ]))
codecs[None] = codecs['NONE']  # pythonic alias

# Aliases they defined
codecs['H265'] = codecs['HEVC']
codecs['IFF_BYTERUN1'] = codecs['IFF_ILBM']

codecs.update(enum(0x8000, [
    'Y41P',
    'AVRP',
    '012V',
    'AVUI',
    'AYUV',
    'TARGA_Y216',
    'V308',
    'V408',
    'YUV4',
    'AVRN',
    'CPIA',
    'XFACE',
    'SNOW',
    'SMVJPEG',
    'APNG',
    'DAALA',
    'CFHD',
    'TRUEMOTION2RT',
    'M101',
    'MAGICYUV',
    'SHEERVIDEO',
    'YLC',
    'PSD',
    'PIXLET',
    'SPEEDHQ',
    'FMVC',
    'SCPR',
    'CLEARVIDEO',
    'XPM',
    'AV1',
    'BITPACKED',
    'MSCC',
    'SRGC',
    'SVG',
    'GDV',
    'FITS',
    'IMM4',
    'PROSUMER',
    'MWSC',
    'WCMV',
    'RASC',
    ]))

codecs.update(enum(0x10000, [
    # /* various PCM "codecs" */
    'PCM_S16LE',
    'PCM_S16BE',
    'PCM_U16LE',
    'PCM_U16BE',
    'PCM_S8',
    'PCM_U8',
    'PCM_MULAW',
    'PCM_ALAW',
    'PCM_S32LE',
    'PCM_S32BE',
    'PCM_U32LE',
    'PCM_U32BE',
    'PCM_S24LE',
    'PCM_S24BE',
    'PCM_U24LE',
    'PCM_U24BE',
    'PCM_S24DAUD',
    'PCM_ZORK',
    'PCM_S16LE_PLANAR',
    'PCM_DVD',
    'PCM_F32BE',
    'PCM_F32LE',
    'PCM_F64BE',
    'PCM_F64LE',
    'PCM_BLURAY',
    'PCM_LXF',
    'S302M',
    'PCM_S8_PLANAR',
    'PCM_S24LE_PLANAR',
    'PCM_S32LE_PLANAR',
    'PCM_S16BE_PLANAR',
    ]))

codecs.update(enum(0x10800, [
    'PCM_S64LE',
    'PCM_S64BE',
    'PCM_F16LE',
    'PCM_F24LE',
    'PCM_VIDC',
    ]))

codecs.update(enum(0x11000, [
    # /* various ADPCM codecs */
    'ADPCM_IMA_QT',
    'ADPCM_IMA_WAV',
    'ADPCM_IMA_DK3',
    'ADPCM_IMA_DK4',
    'ADPCM_IMA_WS',
    'ADPCM_IMA_SMJPEG',
    'ADPCM_MS',
    'ADPCM_4XM',
    'ADPCM_XA',
    'ADPCM_ADX',
    'ADPCM_EA',
    'ADPCM_G726',
    'ADPCM_CT',
    'ADPCM_SWF',
    'ADPCM_YAMAHA',
    'ADPCM_SBPRO_4',
    'ADPCM_SBPRO_3',
    'ADPCM_SBPRO_2',
    'ADPCM_THP',
    'ADPCM_IMA_AMV',
    'ADPCM_EA_R1',
    'ADPCM_EA_R3',
    'ADPCM_EA_R2',
    'ADPCM_IMA_EA_SEAD',
    'ADPCM_IMA_EA_EACS',
    'ADPCM_EA_XAS',
    'ADPCM_EA_MAXIS_XA',
    'ADPCM_IMA_ISS',
    'ADPCM_G722',
    'ADPCM_IMA_APC',
    'ADPCM_VIMA',
    ]))

codecs.update(enum(0x11800, [
    'ADPCM_AFC',
    'ADPCM_IMA_OKI',
    'ADPCM_DTK',
    'ADPCM_IMA_RAD',
    'ADPCM_G726LE',
    'ADPCM_THP_LE',
    'ADPCM_PSX',
    'ADPCM_AICA',
    'ADPCM_IMA_DAT4',
    'ADPCM_MTAF',
    ]))

codecs.update(enum(0x12000, [
    # /* AMR */
    'AMR_NB',
    'AMR_WB',
    ]))

codecs.update(enum(0x13000, [
    # /* RealAudio codecs*/
    'RA_144',
    'RA_288',
    ]))

codecs.update(enum(0x14000, [
    # /* various DPCM codecs */
    'ROQ_DPCM',
    'INTERPLAY_DPCM',
    'XAN_DPCM',
    'SOL_DPCM',
    ]))

codecs.update(enum(0x14800, [
    'SDX2_DPCM',
    'GREMLIN_DPCM',
    ]))

codecs.update(enum(0x15000, [
    # /* audio codecs */
    'MP2',
    'MP3', # ///< preferred ID for decoding MPEG audio layer 1, 2 or 3
    'AAC',
    'AC3',
    'DTS',
    'VORBIS',
    'DVAUDIO',
    'WMAV1',
    'WMAV2',
    'MACE3',
    'MACE6',
    'VMDAUDIO',
    'FLAC',
    'MP3ADU',
    'MP3ON4',
    'SHORTEN',
    'ALAC',
    'WESTWOOD_SND1',
    'GSM', # ///< as in Berlin toast format
    'QDM2',
    'COOK',
    'TRUESPEECH',
    'TTA',
    'SMACKAUDIO',
    'QCELP',
    'WAVPACK',
    'DSICINAUDIO',
    'IMC',
    'MUSEPACK7',
    'MLP',
    'GSM_MS', # /* as found in WAV */
    'ATRAC3',
    'APE',
    'NELLYMOSER',
    'MUSEPACK8',
    'SPEEX',
    'WMAVOICE',
    'WMAPRO',
    'WMALOSSLESS',
    'ATRAC3P',
    'EAC3',
    'SIPR',
    'MP1',
    'TWINVQ',
    'TRUEHD',
    'MP4ALS',
    'ATRAC1',
    'BINKAUDIO_RDFT',
    'BINKAUDIO_DCT',
    'AAC_LATM',
    'QDMC',
    'CELT',
    'G723_1',
    'G729',
    '8SVX_EXP',
    '8SVX_FIB',
    'BMV_AUDIO',
    'RALF',
    'IAC',
    'ILBC',
    'OPUS',
    'COMFORT_NOISE',
    'TAK',
    'METASOUND',
    'PAF_AUDIO',
    'ON2AVC',
    'DSS_SP',
    'CODEC2',
    ]))

codecs.update(enum(0x15800, [
    'FFWAVESYNTH',
    'SONIC',
    'SONIC_LS',
    'EVRC',
    'SMV',
    'DSD_LSBF',
    'DSD_MSBF',
    'DSD_LSBF_PLANAR',
    'DSD_MSBF_PLANAR',
    '4GV',
    'INTERPLAY_ACM',
    'XMA1',
    'XMA2',
    'DST',
    'ATRAC3AL',
    'ATRAC3PAL',
    'DOLBY_E',
    'APTX',
    'APTX_HD',
    'SBC',
    'ATRAC9',
    ]))

codecs.update(enum(0x17000, [
    # /* subtitle codecs */
    'DVD_SUBTITLE',
    'DVB_SUBTITLE',
    'TEXT', #  ///< raw UTF-8 text
    'XSUB',
    'SSA',
    'MOV_TEXT',
    'HDMV_PGS_SUBTITLE',
    'DVB_TELETEXT',
    'SRT',
    ]))

codecs.update(enum(0x17800, [
    'MICRODVD',
    'EIA_608',
    'JACOSUB',
    'SAMI',
    'REALTEXT',
    'STL',
    'SUBVIEWER1',
    'SUBVIEWER',
    'SUBRIP',
    'WEBVTT',
    'MPL2',
    'VPLAYER',
    'PJS',
    'ASS',
    'HDMV_TEXT_SUBTITLE',
    'TTML',
    ]))

codecs.update(enum(0x18000, [
    # /* other specific kind of codecs (generally used for attachments) */
    # 'FIRST_UNKNOWN = 0x18000,           ///< A dummy ID pointing at the start of various fake codecs.
    'TTF',
    'SCTE_35', # ///< Contain timestamp estimated through PCR of program stream.
    ]))

codecs.update(enum(0x18800, [
    'BINTEXT',
    'XBIN',
    'IDF',
    'OTF',
    'SMPTE_KLV',
    'DVD_NAV',
    'TIMED_ID3',
    'BIN_DATA',
    ]))

codecs.update(enum(0x19000, [
    'PROBE', # = 0x19000, ///< codec_id is not known (like 'NONE) but lavf should attempt to identify it
    ]))

codecs.update(enum(0x20000, [
    'MPEG2TS', # = 0x20000, /**< _FAKE_ codec to indicate a raw MPEG-2 TS
                         #              * stream (only used by libavformat) */
    ]))
codecs.update(enum(0x20001, [
    'MPEG4SYSTEMS', # = 0x20001, /**< _FAKE_ codec to indicate a MPEG-4 Systems
                              #              * stream (only used by libavformat) */
    ]))
codecs.update(enum(0x21000, [
    'FFMETADATA', # = 0x21000,   ///< Dummy codec for streams containing only metadata information.
    'WRAPPED_AVFRAME', # = 0x21001, ///< Passthrough codec', AVFrames wrapped in AVPacket
    ]))

PROP_INTRA_ONLY = 1 << 0
PROP_LOSSY = 1 << 1
PROP_LOSSLESS = 1 << 2
PROP_REORDER = 1 << 3
PROP_BITMAP_SUB = 1 << 16
PROP_TEXT_SUB = 1 << 17
BUFFER_PADDING_SIZE = 64  # Should this keep INPUT?
BUFFER_MIN_SIZE = 16384   # Should this keep INPUT?
